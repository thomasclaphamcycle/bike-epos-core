...
- browser-based document print now includes practical office-style outputs for thermal receipts, A4 sales invoices, and bike-tag preview routes, while controlled helper-driven direct print now covers Zebra shipment labels, Dymo product labels, and office-printer bike tags
...
- provider-backed web-order shipping now also has initial lifecycle hardening for provider refresh, supported void/cancel flow, and replacement-shipment recovery
