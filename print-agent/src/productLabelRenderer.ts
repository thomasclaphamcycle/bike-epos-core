...
import type { ProductLabelPrintRequest } from "../../shared/productLabelPrintContract";
...
const LABEL_WIDTH_PX = mmToPx(LABEL_WIDTH_MM);
const LABEL_HEIGHT_PX = mmToPx(LABEL_HEIGHT_MM);
const BARCODE_REGION_HEIGHT = 120;
const BARCODE_TEXT_HEIGHT = 28;
const BARCODE_REGION_TOP = LABEL_HEIGHT_PX - PADDING_Y - BARCODE_REGION_HEIGHT - BARCODE_TEXT_HEIGHT;
...
export const renderProductLabelDocument = (
  request: ProductLabelPrintRequest,
): RenderedProductLabelDocument => {
  const canvas = createCanvas(LABEL_WIDTH_PX, LABEL_HEIGHT_PX);
  const ctx = canvas.getContext("2d");
  const barcodeValue = sanitizeBarcode(request.label.barcode);
  const rightEdge = LABEL_WIDTH_PX - PADDING_X;
  const contentWidth = LABEL_WIDTH_PX - PADDING_X * 2;
  ...
  ctx.font = SHOP_FONT;
  ctx.fillText(fitText(ctx, request.label.shopName, contentWidth - 140), PADDING_X, PADDING_Y);

  ctx.font = PRICE_FONT;
  const priceText = formatMoney(request.label.pricePence);
  const priceWidth = ctx.measureText(priceText).width;
  ctx.fillText(priceText, rightEdge - priceWidth, PADDING_Y - 4);

  const secondaryParts = [request.label.brand, request.label.sku].filter(Boolean);
  ...
  ctx.font = TITLE_FONT;
  ctx.fillStyle = "#111111";
  const title = request.label.variantName
    ? `${request.label.productName} · ${request.label.variantName}`
    : request.label.productName;
  ...
  if (barcodeValue) {
    ...
    ctx.fillText(barcodeText, Math.max(PADDING_X, (LABEL_WIDTH_PX - barcodeTextWidth) / 2), LABEL_HEIGHT_PX - PADDING_Y - BARCODE_TEXT_HEIGHT + 4);
  } else {
