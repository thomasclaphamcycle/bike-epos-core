...
const run = async () => {
  ...
  try {
    ...
    if (startedServer && serverProcess) {
      ...
      try {
        log(
          `Sending SIGTERM to ${serverUsesProcessGroup ? "process group" : "process"} ${serverProcess.pid}`,
        );
        ...
        log(
          `Sending SIGKILL to ${serverUsesProcessGroup ? "process group" : "process"} ${serverProcess.pid}`,
        );
      ...
      const shutdownMs = await waitForServerShutdown(5000);
