...
const sourceDir = path.join(repoRoot, "print-agent", "windows-bike-tag-agent");
const launcherPath = path.join(sourceDir, "corepos-bike-tag-agent-launcher.js");
const readmePath = path.join(sourceDir, "README.txt");
const configExamplePath = path.join(sourceDir, "corepos-bike-tag-agent.config.example.json");
const startScriptPath = path.join(sourceDir, "Start-CorePOSBikeTagAgent.ps1");
const printScriptPath = path.join(repoRoot, "print-agent", "scripts", "print_bike_tag_windows.ps1");
const defaultOutDir = path.join(repoRoot, "tmp", "bike-tag-agent-bundle", `corepos-bike-tag-agent-v${packageJson.version}`);
const defaultBuildDir = path.join(repoRoot, "tmp", "bike-tag-agent-build", `corepos-bike-tag-agent-v${packageJson.version}`);
const defaultTarget = "node18-win-x64";

const executableFileName = "corepos-bike-tag-agent.exe";
...
const main = async () => {
  ...
  await fs.copyFile(
    configExamplePath,
    path.join(options.outDir, "corepos-bike-tag-agent.config.example.json"),
  );
  ...
  if (options.skipExeBuild) {
    await fs.writeFile(
      ...
      [
        ...
        "Re-run npm run print-agent:package:bike-tag without --skip-exe-build on a packaging machine to generate corepos-bike-tag-agent.exe.",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    const launcherBuildPath = path.join(options.buildDir, "corepos-bike-tag-agent-launcher.js");
  ...
  console.log(`[bike-tag-agent-package] Bundle ready at ${options.outDir}`);
};
