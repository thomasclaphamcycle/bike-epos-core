...
const outDir = path.join(repoRoot, 'tmp', 'zebra-shipment-agent-bundle-smoke');

const runPackager = () =>
  new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ['scripts/package_zebra_shipment_agent.js', '--out-dir', outDir, '--skip-exe-build'],
      {
        cwd: repoRoot,
        stdio: 'inherit',
      },
...
const run = async () => {
  ...
  try {
    ...
    assert.equal(manifest.name, 'corepos-zebra-shipment-agent');
    ...
    assert.deepEqual(manifest.runtimeFiles, [
      'corepos-zebra-shipment-agent.exe',
      'corepos-zebra-shipment-agent.config.example.json',
      'README.txt',
    ]);
    ...
    assert.match(readme, /corepos-zebra-shipment-agent\.exe/i);
    assert.match(readme, /COREPOS_SHIPPING_PRINT_AGENT_URL/);
    assert.match(readme, /WINDOWS_PRINTER/i);

    const configExample = JSON.parse(
      await fs.readFile(path.join(outDir, 'corepos-zebra-shipment-agent.config.example.json'), 'utf8'),
    );
    assert.equal(configExample.port, 3211);
    assert.equal(configExample.bindHost, '127.0.0.1');

    console.log('zebra shipment agent exe packaging layout passed');
  } finally {
